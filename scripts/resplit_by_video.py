...
def read_class_names(labels_dir: Path) -> list[str]:
    ...
    # CVAT YOLO 1.1 export: class names live in a per-cam obj.names.
    for sub in sorted(labels_dir.glob("*/obj.names")) + sorted(labels_dir.glob("*/classes.txt")):
        lines = [ln.strip() for ln in sub.read_text(encoding="utf-8").splitlines() if ln.strip()]
        if lines:
            return lines
    sys.exit(f"No class names found in {labels_dir} (need classes.txt, data.yaml, "
             "or a per-cam obj.names from a CVAT export).")


def cam_label_files(cam_dir: Path) -> list[Path]:
    """Label .txt for a cam, whether flat (<cam>/*.txt) or CVAT-nested
    (<cam>/obj_train_data/*.txt). Ignores CVAT's obj.names/train.txt sidecars."""
    flat = [p for p in cam_dir.glob("*.txt") if p.name not in ("train.txt",)]
    nested = list((cam_dir / "obj_train_data").glob("*.txt"))
    return sorted(nested or flat)
...
def main() -> None:
    ...
    for cam_dir in cam_dirs:
        ...
        for txt in cam_label_files(cam_dir):
    ...
    # Auto-drop a trailing zero-instance placeholder (e.g. "bag" fully reclassified to
    # subclasses) -> clean 4-class dataset. Other ids are unaffected (bag was last).
    dropped_placeholder = (placeholder_id is not None and placeholder_id == len(names) - 1
                           and placeholder_hits == 0)
    out_names = names[:-1] if dropped_placeholder else names
    ...
    if dropped_placeholder:
        print(f"(dropped empty placeholder class '{PLACEHOLDER_CLASS}' -> nc={len(out_names)})")
